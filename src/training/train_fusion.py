"""
Training entry point for the fusion model.
...
def main(config_path: str):
    ...
    train_loader = get_dataloader(task="fusion", split="train", config=config,
                                   batch_size=config["training"]["batch_size"])
    val_loader = get_dataloader(task="fusion", split="val", config=config,
                                 batch_size=config["training"]["batch_size"])

    model = FusionModel(config=config)
    # For fine-tuning from a pretrained backbone, load it here before training:
    # model.load(config["models"]["fusion"]["checkpoint"], device=device)
    optimizer = build_optimizer(model, config)
    logger = SimpleLogger(log_file=f"models/checkpoints/fusion_train.log")
    ...
    for epoch in range(config["training"]["epochs"]):
        ...
        save_checkpoint(model, config["models"]["fusion"]["checkpoint"], epoch, optimizer)
