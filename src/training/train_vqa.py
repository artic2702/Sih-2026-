"""
Training entry point for the vqa model.
...
from src.models.vqa_model import VQAModel, train_step
...
def main(config_path: str):
    ...
    train_loader = get_dataloader(task="vqa", split="train", config=config,
                                   batch_size=config["training"]["batch_size"])
    val_loader = get_dataloader(task="vqa", split="val", config=config,
                                 batch_size=config["training"]["batch_size"])

    model = VQAModel(config=config)
    # For fine-tuning from a pretrained backbone, load it here before training:
    # model.load(config["models"]["vqa"]["checkpoint"], device=device)
    optimizer = build_optimizer(model, config)
    logger = SimpleLogger(log_file=f"models/checkpoints/vqa_train.log")
    ...
    for epoch in range(config["training"]["epochs"]):
        ...
        save_checkpoint(model, config["models"]["vqa"]["checkpoint"], epoch, optimizer)
